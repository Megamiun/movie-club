...
- [x] Show a unfilled shape similar to the current for missing ratings
  - A dashed-outline shape, colored with the member's own color instead of a generic gray. Per follow-up feedback ("a little less compact, at least one square for each"), quality and sentiment each render as their own small square side by side instead of being merged into a single split-color dot -- `InlineRatingEditor`'s `RatingSquare` helper, 18px each
