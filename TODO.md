...
- [x] Meetings pane should show film data similar to the CSV file (director, runtime, genre, etc.)
  ...
  - Episode rows are grouped under their series' title (the acting club's own pick's title, resolved via `EpisodeRepository.findSeriesTitle`), shown once before that series' first episode in a given meeting rather than repeated per row
