...
- [x] Meetings pane should show film data similar to the CSV file (director, runtime, genre, etc.)
  ...
  - Episode rows are grouped under a full series header row (not just a title) shown once before that series' first episode in a given meeting -- chosen-by, title+IMDB link, year, creator, genre, country, and rating, matching a movie pick's own level of detail (genre/country used to show as "—" on every episode row, since those are series-level facts an Episode doesn't itself carry). `MeetingEpisodePick.series` is the club's own full pick (`SeriesRow`), resolved via `EpisodeRepository.findSeriesImdbId` + `SeriesRepository.findByClubAndImdbId` -- kept as two repository calls composed in `MeetingService` rather than one cross-repository lookup, per the "repositories never depend on other repositories" rule
