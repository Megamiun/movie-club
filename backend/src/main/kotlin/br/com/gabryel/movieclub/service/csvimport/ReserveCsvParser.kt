...
import br.com.gabryel.movieclub.service.csvimport.WatchlistCategory.MOVIE
import br.com.gabryel.movieclub.service.csvimport.WatchlistCategory.SERIES
 ...
 * a `Choice` cell: a 2-row merged header (`Movies,,Series,` then `Person A,Person B,Person A,Person B`) defines 4 ragged
...
object ReserveCsvParser {
    fun parse(input: InputStream): List<ReserveCsvRow> {
        ...
        // ("Movies,,Series," then "Person A,Person B,Person A,Person B"), not the single-header-row shape
        ...
        val columns = (0 until displayNameRow.size()).mapNotNull { index ->
            ...
            val category = if (index < 2) MOVIE else SERIES
        ...
        return records.drop(2).flatMap { record ->
            columns.mapNotNull { (index, categoryAndName) ->
                ...
                val title = naToNull(record.get(index))?.trim()
                    ?: return@mapNotNull null
