...
class MeetingService(
    ...
    fun createMeeting(clubId: Uuid, actingMemberId: Uuid, date: LocalDate, assignedMemberId: Uuid? = null): MeetingRow {
        ...
        if (assignedMemberId != null) clubService.requireMembership(clubId, assignedMemberId)
    ...
    fun swapAssignments(meetingIdA: Uuid, meetingIdB: Uuid, actingMemberId: Uuid): Pair<MeetingRow, MeetingRow> {
        ...
        if (a.clubId != b.clubId) throw BadRequestException("Meetings must belong to the same club")
    ...
    fun mergeMeetings(intoMeetingId: Uuid, fromMeetingId: Uuid, actingMemberId: Uuid): MeetingRow {
        ...
        if (into.clubId != from.clubId) throw BadRequestException("Meetings must belong to the same club")

        movieRepository.listByMeeting(from.id).forEach { movieRepository.updateMeeting(it.id, into.id) }
        meetingRepository.delete(from.id)
        return meetingRepository.updateAssignedMember(into.id, null)
    }
