...
class ExposedClubRepository : ClubRepository {
    override fun create(name: String): ClubRow =
        transaction {
    ...
    override fun addMember(clubId: Uuid, memberId: Uuid, role: ClubRole, rotationOrder: Int): ClubMembershipRow =
        transaction {
    ...
    override fun findMembership(clubId: Uuid, memberId: Uuid): ClubMembershipRow? =
        transaction {
            ClubMembers
                .selectAll()
    ...
    override fun listMembers(clubId: Uuid): List<ClubMembershipRow> =
        transaction {
            ClubMembers
                .selectAll()
    ...
    override fun listClubsForMember(memberId: Uuid): List<ClubRow> =
        transaction {
    ...
    override fun updateRole(clubId: Uuid, memberId: Uuid, role: ClubRole): ClubMembershipRow =
        transaction {
    ...
    override fun updateRotationOrder(clubId: Uuid, memberId: Uuid, rotationOrder: Int): ClubMembershipRow =
        transaction {
            ClubMembers.update({ (ClubMembers.clubId eq clubId) and (ClubMembers.memberId eq memberId) }) {
                it[ClubMembers.rotationOrder] = rotationOrder
            }
    ...
    override fun countAdmins(clubId: Uuid): Int =
        transaction {
            ClubMembers
                .selectAll()
    ...
    private fun toClubRow(row: ResultRow) =
        ClubRow(
    ...
    private fun toMembershipRow(row: ResultRow) =
        ClubMembershipRow(
