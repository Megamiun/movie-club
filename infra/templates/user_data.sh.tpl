...
# Content is rendered once by Terraform itself (infra/main.tf's local.docker_compose_content, from
# templates/docker-compose.yml.tpl) and spliced in here verbatim -- kept in one file rather than duplicated
# between this first-boot copy and the docker_compose_content output terraform.yml's apply job pushes to an
# already-running instance via SSM (see that output's own comment for why the SSM push exists at all).
