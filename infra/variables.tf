variable "project_name" {
  description = "Naming prefix for resources whose names can't collide across AWS accounts/deployments -- currently just the two S3 buckets (s3_frontend.tf, s3_backups.tf), named \"<project_name>-frontend\"/\"<project_name>-backups\". S3 bucket names are globally unique across *all* of AWS, not just this account, and there's no account-id suffix appended here to guarantee that -- project_name alone has to already be distinctive enough (e.g. include your own domain or account name) or bucket creation just fails outright on a collision with someone else's bucket. Defaults to \"movie-club\", matching what was already hardcoded into both bucket names before this variable existed, but that default is not itself guaranteed unique -- set your own value in terraform.tfvars for a real deployment. Changing this value for an already-applied deployment renames both buckets, which Terraform can only do by destroying and recreating them (S3 bucket names are immutable) -- never change it for a live deployment without deliberately planning for that (see infra/README.md's Backups section)."
  type        = string
  default     = "movie-club"
}
